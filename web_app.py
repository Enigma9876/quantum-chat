...
@app.route('/local')
def local(): 
    return render_template('index.html', page='local', active_usernames=active_usernames)
...
def host_server():
    ...
    session['room'] = code
    return render_template('index.html', page='chat', room_code=code, current_user=session['username'])
